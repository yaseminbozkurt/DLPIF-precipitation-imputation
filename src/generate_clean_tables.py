...
def main():
    ...
    METHODS = {scen_label: {} for scen_label, _ in SCENARIOS}

    # Raw GAN seeds, scenario-specific
    for seed in [42,123,456]:
        for scen_label, _mask_key in SCENARIOS:
            p = os.path.join(OUTPUT_DIR, f'gan_imputed_test_modeB_{scen_label}_seed{seed}.npy')
            if os.path.exists(p):
                METHODS[scen_label][f'WGAN-GP_raw_seed{seed}'] = p
            else:
                print(f'  [WARN] WGAN-GP raw missing: {os.path.basename(p)}')

    # PrecipFix, scenario-specific
    for seed in [42,123,456]:
        for scen_label, _mask_key in SCENARIOS:
            p = os.path.join(OUTPUT_DIR, f'gan_imputed_test_modeB_{scen_label}_seed{seed}_precipfix.npy')
            if os.path.exists(p):
                METHODS[scen_label][f'PrecipFix_seed{seed}'] = p

    # Check for leftover old agnostic files from before the bug fix, warn if found
    for seed in [42,123,456]:
        p_old_p2s = os.path.join(OUTPUT_DIR, f'gan_imputed_test_modeB_seed{seed}_msclean_precip2stage.npy')
        p_old_dlpif = os.path.join(OUTPUT_DIR, f'gan_imputed_test_modeB_seed{seed}_msclean_amountrf.npy')
        if os.path.exists(p_old_p2s):
            print(f'  [WARNING] Obsolete scenario-agnostic file detected (ignored): {os.path.basename(p_old_p2s)}')
        if os.path.exists(p_old_dlpif):
            print(f'  [WARNING] Obsolete scenario-agnostic file detected (ignored): {os.path.basename(p_old_dlpif)}')

    print('  Scenario-specific methods found:')
    for scen_label in SCEN_LABELS:
        print(f'    {scen_label}: {list(METHODS.get(scen_label, {}).keys())}')
    ...
    for scen_label, mask_key in SCENARIOS:
        ...
        # GAN-based methods (scenario-specific)
        for mname, npy_path in METHODS.get(scen_label, {}).items():
        ...
        # DLPIF scenario-specific methods (T1.1 bug fix)
        for seed in [42,123,456]:
            # Precip2Stage
            p_p2s = os.path.join(OUTPUT_DIR, f'gan_imputed_test_modeB_seed{seed}_msclean_precip2stage_{scen_label}.npy')
            if not os.path.exists(p_p2s):
                raise FileNotFoundError(f"Missing required scenario-specific file: {os.path.basename(p_p2s)}. Run multiseed_clean_rerun.py first.")
            pred_mm = load_npy_precip_mm(sc, p_p2s, pidx)[m]
            row = precip_cls(gt_m, pred_mm)
            row.update(extreme_metrics(gt_m, pred_mm))
            row.update(method=f'Precip2Stage_clean_seed{seed}', scenario=scen_label, n_masked=n_m)
            records.append(row)

            # AmountRF/DLPIF
            p_amt = os.path.join(OUTPUT_DIR, f'gan_imputed_test_modeB_seed{seed}_msclean_amountrf_{scen_label}.npy')
            if not os.path.exists(p_amt):
                raise FileNotFoundError(f"Missing required scenario-specific file: {os.path.basename(p_amt)}. Run multiseed_clean_rerun.py first.")
            pred_mm = load_npy_precip_mm(sc, p_amt, pidx)[m]
            row = precip_cls(gt_m, pred_mm)
            row.update(extreme_metrics(gt_m, pred_mm))
            row.update(method=f'AmountRF_clean_seed{seed}', scenario=scen_label, n_masked=n_m)
            records.append(row)
